...
> string getAnalyticsGoalReportPerDevicePerChannelGroupPerOrganisationPerViewPerDay($organisation_uuids, $from_date, $date_format, $format)
...
try {
    $result = $apiInstance->getAnalyticsGoalReportPerDevicePerChannelGroupPerOrganisationPerViewPerDay($organisation_uuids, $from_date, $date_format, $format);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AnalyticsDataGoalsApi->getAnalyticsGoalReportPerDevicePerChannelGroupPerOrganisationPerViewPerDay: ', $e->getMessage(), PHP_EOL;
}
?>
```
