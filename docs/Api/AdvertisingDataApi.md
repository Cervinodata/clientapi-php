...
```php
getAdformExtendedReportPerOrganisationPerCampaignPerDay($organisation_uuids, $from_date, $date_format, $format): string
```
...
try {
    $result = $apiInstance->getAdformExtendedReportPerOrganisationPerCampaignPerDay($organisation_uuids, $from_date, $date_format, $format);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling AdvertisingDataApi->getAdformExtendedReportPerOrganisationPerCampaignPerDay: ', $e->getMessage(), PHP_EOL;
}
```
